...
public class Fog_Valve_Script : Photon.MonoBehaviour
{
        ...
        if (!photonView.isMine)
        {
            ...
            if (!isCommandSent)
            {
                if (valveTotalRotation > 360)
                {
                    ...
                    photonView.RPC("CmdVentFog", PhotonTargets.MasterClient, null);
                }
    ...
    void CmdVentFog()
    {
        mastermindScript.photonView.RPC("VentFog", PhotonTargets.All, null);
    }
