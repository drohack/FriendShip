...
[RequireComponent(typeof(Vive_VelocityTracker))]
public class Vive_Hand : MonoBehaviour
{
    ...
    private Vive_VelocityTracker m_velocityTracker = null;
    ...
    private void Start()
    {
        ...
        m_velocityTracker = this.GetComponent<Vive_VelocityTracker>();
        ...
        if (m_trackedObject != null && Convert.ToInt32(m_trackedObject.index) != -1)
    ...
    private void Update()
    {
        ...
        if (device != null)
        {
            ...
            m_flex = device.GetPress(SteamVR_Controller.ButtonMask.Grip) ? 1 : device.GetAxis(Valve.VR.EVRButtonId.k_EButton_Axis1).x; //Grip & Trigger
            m_trigger = device.GetAxis(Valve.VR.EVRButtonId.k_EButton_Axis1).x; //Since we combined m_flex to both Grip & Trigger this will always be Zero
            m_point = (device.GetPress(SteamVR_Controller.ButtonMask.Grip) && device.GetAxis(Valve.VR.EVRButtonId.k_EButton_Axis1).x == 0) ? 1 : 0; //If you hold the grip button use the trigger as the pointer
            m_thumbsUp = device.GetTouch(SteamVR_Controller.ButtonMask.Touchpad) ? 0 : 1; //Detect finger over trackpad
