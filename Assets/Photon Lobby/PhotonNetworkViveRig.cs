...
public class PhotonNetworkViveRig : Photon.MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    SteamVR_TrackedObject cameraHead_TrackedObject;
    [SerializeField]
    GUILayer cameraHead_GUILayer;
    [SerializeField]
    Camera eye_Camera;
    [SerializeField]
    SteamVR_Camera eye_SteamVRCamera;
    [SerializeField]
    FlareLayer eye_FlareLayer;
    [SerializeField]
    AudioListener ears_AudioListener;
    [SerializeField]
    SteamVR_Ears ears_SteamVREars;
    [SerializeField]
    PhotonVoiceRecorder ears_PhotonVoiceRecorder;
    [SerializeField]
    GameObject LeftHandPf_GO;
    [SerializeField]
    Transform LeftHandPf;
    [SerializeField]
    Vive_Hand handScriptL;
    [SerializeField]
    SteamVR_TrackedObject trackedObjectL;
    [SerializeField]
    Vive_VelocityTracker velocityTrackerL;
    [SerializeField]
    ...
    [SerializeField]
    GameObject RightHandPf_GO;
    [SerializeField]
    Transform RightHandPf;
    [SerializeField]
    Vive_Hand handScriptR;
    [SerializeField]
    SteamVR_TrackedObject trackedObjectR;
    [SerializeField]
    Vive_VelocityTracker velocityTrackerR;
    [SerializeField]
    ...
    void Start()
    {
        if (photonView.isMine)
        {
            ...
            RecenterController recenterController = gameObject.AddComponent<RecenterController>();
            ...
            eye_Camera.enabled = true;
            eye_SteamVRCamera.enabled = true;
            eye_FlareLayer.enabled = true;
            ears_AudioListener.enabled = true;
            ears_SteamVREars.enabled = true;
            ears_PhotonVoiceRecorder.enabled = true;
            trackedObjectL.enabled = true;
    ...
    void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.isWriting)
        {
            ...
            stream.SendNext(cameraHead_Transform.position);
            stream.SendNext(cameraHead_Transform.rotation);
            if(leftIsEnabled)
            {
                oldLeftHandPf = null;
    ...
    void Update()
    {
        if (!photonView.isMine)
        {
            ...
            cameraHead_Transform.position = Vector3.Lerp(cameraHead_Transform.position, centerEyeAnchorPos, Time.deltaTime * 20);
            cameraHead_Transform.rotation = Quaternion.Lerp(cameraHead_Transform.rotation, centerEyeAnchorRot, Time.deltaTime * 20);
        ...
        if (ears_PhotonVoiceRecorder != null && ears_PhotonVoiceRecorder.enabled && !ears_PhotonVoiceRecorder.Transmit)
        {
            //Debug.Log("Turn On Transmit");
            ears_PhotonVoiceRecorder.Transmit = true;
        }
    }
